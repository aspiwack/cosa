...
VFILES:=Interaction/Interaction.v\
  Interaction/InteractionLib.v\
  Interaction/Rule.v\
  Interaction/Transition.v\
  Interaction/Simulation.v\
  Shape/ShapeDomain.v\
  Shape/Graph.v\
  Shape/Inclusion.v\
  Shape/Summary.v\
  Concrete/ConcreteFragment.v\
  Abstract/Valuation.v\
  Abstract/NumericalDomain.v\
  Abstract/Lang.v\
  Lib/Extra.v\
  Lib/MapReduce.v\
  Lib/Algebra.v\
...
ML4FILES:=Plugins/g_eresolve.ml4

-include $(addsuffix .d,$(ML4FILES))
.SECONDARY: $(addsuffix .d,$(ML4FILES))

MLFILES:=Plugins/eresolve.ml\
  Plugins/combinatorize.ml
...
install-natdynlink:
	for i in $(CMXSFILES); do \
	 install -d `dirname $(DSTROOT)$(COQLIBINSTALL)/Cosa/$$i`; \
	 install -m 0644 $$i $(DSTROOT)$(COQLIBINSTALL)/Cosa/$$i; \
	done
...
install-doc:
	install -d $(DSTROOT)$(COQDOCINSTALL)/Cosa/html
	for i in html/*; do \
	 install -m 0644 $$i $(DSTROOT)$(COQDOCINSTALL)/Cosa/$$i;\
	done
